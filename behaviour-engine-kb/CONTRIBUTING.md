...
```yaml
---
id: what-is-behaviour-engine      # 小写连字符，必须与文件名一致
...
1. `category` **必须等于目录名**（索引按目录过滤）。
2. `id` **必须等于文件名**（去 `.md`）。
3. 六个必填字段（`id/title/category/version/updated/tags/source`）+ `summary` 不得为空。
...
1. 建新目录（建议带序号前缀，如 `08-xxx`）；
2. 在 `manifest.json` 的 `categories[]` 增加一项：`{ "dir": "08-xxx", "title": "中文名", "desc": "..." }`；
3. 重跑构建脚本。

## 三、来源纪律（本库特有，务必遵守）

- **原理/版本/路径**：一律以官方页面为准，并在正文标注 `> 来源：URL`。
- **社区说法**：单独标注"（社区）"，不得与官方口径混写。
- **禁止**引用 CSDN / toolify 等内容农场/AI 聚合站的版本号与功能名（见 [`07-sources/unreliable-sources.md`](07-sources/unreliable-sources.md)）。
- 抓来的上游原文放 `_raw/`，**不参与索引**。
